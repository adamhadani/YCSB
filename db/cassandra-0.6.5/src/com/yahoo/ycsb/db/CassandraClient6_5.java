...
/**
 * Cassandra 0.65 client for YCSB framework
 */
public class CassandraClient6_5 extends DB
{
	...
	public int readMultiple(String table, Set<String> keys, Set<String> fields, List<Map<String, String>> result) {
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			try
			{
				...
				{
					final List<byte[]> fieldList = new ArrayList<byte[]>();
					for (final String field : fields)
					{
						fieldList.add(field.getBytes("UTF-8"));
					}
					predicate.setColumn_names(fieldList);
				}
				final ColumnParent parent = new ColumnParent("data");
				...
				final Map<String,List<ColumnOrSuperColumn>> rows = client.multiget_slice(table, new ArrayList<String>(keys), parent, predicate, ConsistencyLevel.ONE);
				for (final List<ColumnOrSuperColumn> row : rows.values())
				{
					final Map<String,String> tuple = new HashMap<String,String>();
					for (final ColumnOrSuperColumn columnOrSuperColumn : row)
					{
						final Column column = columnOrSuperColumn.getColumn();
						final String columnName = new String(column.getName());
						final String columnValue = new String(column.getValue());
						tuple.put(columnName, columnValue);
						if (_debug)
						{
	...
	{
		CassandraClient6_5 cli=new CassandraClient6_5();
